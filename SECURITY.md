...
- **Optional Client-Mediated Confirmation (Unreleased)**: `--workspace-write-confirmation` / `MCP_WORKSPACE_WRITE_CONFIRMATION` adds a default-off MCP handler gate to both mutation tools. Only schema-validated accepted `confirm: true` responses proceed; decline, cancel, false, and invalid accepted content do not mutate files. The prompt contains only the validated logical target and operation, with escaped controls and no silent path truncation. Arguments and resolved logical target participate in the response key, but this digest is **not** an authorization token or replay protection. A malicious client can fabricate an acceptance; use trusted clients that actually ask a human. Direct service calls are not covered by the MCP confirmation gate.
